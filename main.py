                ...
                while std_grade_del_id_len==9:
                    ...
                    class Database:
                        def __init__(self):
                            ...
                            self.filtered_out_ass = []
                            self.filtered_out_quiz = []
                            self.filtered_out_exam = []
                            self.subject_del_copy = []
                            self.del_type = ""
                            self.del_out_where = 0

                        def find_student(self, std_grade_del_int):
                            found = 0
                            for std in self.std_info:
                                if std[0] == std_grade_del_int:
                                    ...
                                    print(f'\n   [OK]   ID: {std[0]} Found')
                                    time.sleep(2)
                            if self.std_info == f"{sample_std}":
                                print('Sample mode')
                                any_key()
                                found = 0
                            elif self.std_info == [] or None:
                                print('No registered ID in this database once!')
                                any_key()
                                found = 0
                            else:
                            ...
                            while found == 1:
                                if out_ctl == 0:
                                    break
                                cur.execute(f"SELECT * FROM {t_std_sub}")
                                self.course_read = cur.fetchall()
                                print('\nAvailable subjects:')
                                print('ID | Subject')
                                for sub in self.course_read:
                                    print(f'{sub[0]}  | {sub[1]}')
                                while True:
                                    match = 0
                                    try:
                                ...
                                while True:
                                    ...
                                    out_ctl = 1
                                    clear()
                                    self.filtered_out_data = [item for item in self.search_output if item[5] == std_grade_del_sub_data]
                                    self.filtered_out_ass = [item for item in self.search_output if item[1] == 'assignment']
                                    self.filtered_out_quiz = [item for item in self.search_output if item[1] == 'quiz']
                                    self.filtered_out_exam = [item for item in self.search_output if item[1] == 'exam']
                                    for out in self.filtered_out_ass + self.filtered_out_quiz + self.filtered_out_exam:
                                    ...
                                    while True:
                                        ...
                                        if del_cur == 'a':
                                            err = input(self.filtered_out_ass)
                                            if self.filtered_out_ass == []:
                                                print('No assignment data output!')
                                            else:
                                                self.del_type = output_type[0]
                                                del_trig = 1
                                        elif del_cur == 'q':
                                            if self.filtered_out_quiz == []:
                                                err = input('No quiz data output!')
                                            else:
                                                self.del_type = output_type[1]
                                                del_trig = 1
                                        elif del_cur == 'e':
                                            if self.filtered_out_exam == []:
                                                err = input('No exam data output!')
                                            else:
                                                self.del_type = output_type[2]
                                                del_trig = 1
                                        elif del_cur == 'fe':
                                            out_ctl = 0
                                            break
                                        while del_trig == 1:
                                            ...
                                            try:
                                                print('')
                                                self.del_out_where = int(input('Output no.: '))
                                                if del_cur == 'a':
                                                    print('Assignment')
                                                    for del_out in self.filtered_out_ass:
                                                        if self.del_out_where == del_out[2]:
                                                            del_trig_inner = 2
                                                        elif self.del_out_where != del_out[2]:
                                                            found_out = 0
                                                elif del_cur == 'q':
                                                    print('Quiz')
                                                    for del_out in self.filtered_out_quiz:
                                                        if self.del_out_where == del_out[2]:
                                                            del_trig_inner = 2
                                                        elif self.del_out_where != del_out[2]:
                                                            found_out = 0
                                                    ...
                                                    print('Exam')
                                                    for del_out in self.filtered_out_exam:
                                                        if self.del_out_where == del_out[2]:
                                                            del_trig_inner = 2
                                                        elif self.del_out_where == del_out[2]:
                                                            found_out = 0
                                                ...
                                                while del_trig_inner == 2:
                                                    del_confirm = input(
                                                        f'\nDelete?\nOutput: {self.del_out_where}\nOutput Type: {self.del_type} @ Course {out[5]}\n::y/N:').lower()
                                                    if del_confirm == 'y':
                                                        cur.execute(
                                                            f"DELETE FROM {t_std_mark} WHERE std_id={std_grade_del_int} AND output_type='{self.del_type}' AND output_no={out[2]}")
                                                        con.commit()
                                                        print('Deleted!')
                                                        time.sleep(2)
                                                        del_trig_inner = 0
                    ...
                    if main_selection == '1':
                        db.find_student(std_grade_del_int)
